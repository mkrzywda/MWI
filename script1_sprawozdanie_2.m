...
% Ustawienie wartości do obliczeń
...
figure(1);
title('Wykres zmiany objętości austenitu w stali w funkcji czasu');
ylabel('Ułamek objętości');
xlabel('Krok');
axis([1 100 0 30]);
hold on;
plot(austenit,'red-');
...
figure(2);
title('Rozkład stężenia z odległością w poszczególnych krokach');
xlabel('element');
ylabel('stężenie[%]');
hold on;
plot(tab(1,1:15));
